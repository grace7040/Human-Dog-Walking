...
/*
 * This is the basic controller. It receives the keycodes from the InputManager and interprets them for BioAnimation_Original
 */
public class Controller
{
    ...
    /*
     * Is called by BioAnimation_Original and returns the running direction and speed
     */
    public virtual Vector3 QueryMove(float x =0f, float z = 0f)
    {
        Vector3 move = Vector3.zero;
        move.x = Input.GetAxis("Horizontal") + x;

        move.z = Input.GetAxis("Vertical") + z;
    ...
    /*
     * Is called by BioAnimation_Original and returns the look direction
     */
    public virtual float QueryTurn()
    {
        float turn = 0f;
        if (InputHandler.GetKey(TurnLeft) || InputHandler.GetKey(KeyCode.Joystick1Button4))
        {
            turn -= 1f;
        }
        if (InputHandler.GetKey(TurnRight) || InputHandler.GetKey(KeyCode.Joystick1Button5))
        {
            turn += 1f;
        }
        return turn;

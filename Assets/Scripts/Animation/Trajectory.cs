...
/*
 * This class represents the trajectory, which consists of points containing the positions and the (running) style.
 */
public class Trajectory {
	...
	public void Draw(int step=1) {
		...
		for(int i=0; i<Points.Length; i+=step) {
			...
			UltiDraw.DrawLine(Points[i].GetPosition(), Points[i].GetPosition() + 0.25f*Points[i].GetDirection(), 0.025f, 0f, UltiDraw.Orange.Transparenty(0.75f));
		}
		...
		*/
		...
		for(int i=0; i<Points.Length; i+=step) {
			...
			UltiDraw.DrawArrow(Points[i].GetPosition(), Points[i].GetPosition() + Points[i].Phase*Vector3.up, 0.8f, 0.025f, 0.05f, UltiDraw.IndianRed.Transparenty(0.5f));

...
public class BioAnimation_Original : MonoBehaviour
{
    ...
    */
    ...
    float x = 0;
    float z = 0;
    public bool isTensioned = false;
    public void ApplyLeashPull(bool isTensioned, float x = 0f, float z = 0f)
    {
        this.x = x;
        this.z = z;
        this.isTensioned = isTensioned;
    }
    void Update()
    {
        //fpsCounter++;
        ...
        Debug.Log($"isTensioned : {isTensioned}");
        Vector3 playerMove = isTensioned ? Controller.QueryMove(x, z) : Controller.QueryMove();
        TargetVelocity = Vector3.Lerp(TargetVelocity, (Quaternion.LookRotation(TargetDirection, Vector3.up) * playerMove).normalized, TargetBlending);
    ...
    void OnRenderObject()
    {
        ...
        */
        ...
        if (Application.isPlaying&& visualizeTrajectory)
        {
            ...
            UltiDraw.Begin();
            UltiDraw.DrawLine(Trajectory.Points[RootPointIndex].GetPosition(), Trajectory.Points[RootPointIndex].GetPosition() + TargetDirection, 0.05f, 0f, UltiDraw.Red.Transparenty(0.75f));
            UltiDraw.DrawLine(Trajectory.Points[RootPointIndex].GetPosition(), Trajectory.Points[RootPointIndex].GetPosition() + TargetVelocity, 0.05f, 0f, UltiDraw.Green.Transparenty(0.75f));
            UltiDraw.End();
            Trajectory.Draw(10);
            ...
            for (int i = 0; i < Actor.Bones.Length; i++)
            {
                UltiDraw.DrawArrow(
                    ...
                    0.0075f,
                    0.05f,
                    UltiDraw.Purple.Transparenty(0.5f)
                );
            }
            UltiDraw.End();
        }
